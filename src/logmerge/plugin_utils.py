...
class LogParsingPlugin:
    ...
    compiled regex patterns, and pre-built lookup tables for performance.
    """
    ...
    def __init__(self, schema_data: Dict[str, Any], parse_function: Callable = None):
        ...
        self.regex_pattern = schema_data.get('regex')  # Optional when custom parser is provided
        ...
        # Pre-build enum lookup dictionaries for performance
        self.enum_lookups = self._build_enum_lookups()
    
    def _build_enum_lookups(self) -> Dict[str, Dict[int, str]]:
        """Build enum lookup dictionaries for performance optimization.
        
        Returns:
            Dictionary mapping field names to their enum value->name lookups
        """
        enum_lookups = {}
        for field in self.fields:
            if field['type'] == 'enum':
                ...
                # Create value -> name lookup dictionary
                enum_lookups[field_name] = {
...
def validate_schema_structure(schema_data: Dict[str, Any], parse_function: Callable = None) -> None:
...
def validate_timestamp_field(schema_data: Dict[str, Any]) -> None:
    ...
    # Check that the timestamp field is epoch or strptime type
    timestamp_field_def = next((field for field in schema_data['fields'] 
                              if field['name'] == timestamp_field), None)
    if timestamp_field_def and timestamp_field_def['type'] not in ['epoch', 'strptime']:
        raise ValueError(f"timestamp_field '{timestamp_field}' must be of type 'epoch' or 'strptime'")
    ...
    if timestamp_field_def and timestamp_field_def['type'] == 'strptime':
        if not timestamp_field_def.get('strptime_fmt'):
            raise ValueError(f"Field '{timestamp_field}' uses 'strptime' but missing 'strptime_fmt'")
