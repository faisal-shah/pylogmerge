...
class LogParsingWorker(QThread):
    ...
    def __init__(self, schema: LogParsingPlugin, shared_buffer: SharedLogBuffer, parent=None):
    ...
    def _check_file_for_new_lines(self, file_path: str, state: FileMonitorState) -> List[LogEntry]:
        ...
        try:
            ...
            if (current_modified <= state.last_modified and 
            ...
            if not state.file_handle:
                state.file_handle = open(file_path, 'r', encoding=DEFAULT_FILE_ENCODING, errors=DEFAULT_FILE_ERROR_HANDLING)
            ...
            self.logger.debug(f"Processed {len(new_entries)} new entries from {file_path} in {execution_time:.6f} seconds")
    ...
    def _parse_line(self, file_path: str, line_number: int, raw_line: str) -> Optional[LogEntry]:
