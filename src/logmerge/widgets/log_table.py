...
class LogTableModel(QAbstractTableModel):
    ...
    def _matches_field_filters(self, entry: LogEntry) -> bool:
        ...
        for field_name, filter_criteria in self.field_filters.items():
            ...
            if filter_type == "discrete":
                ...
                if from_val is not None and (entry_value is None or entry_value < from_val):
                    return False
            ...
            for field_name in self.visible_columns:
                if field_name == self.SOURCE_FILE_COLUMN:
                    ...
                    if isinstance(raw_value, datetime):
                        ...
                        if (hasattr(self.schema, 'enum_display_maps') and 
                            field_name in self.schema.enum_display_maps):
                            # Use pre-built enum display map - O(1) lookup!
                            enum_display_map = self.schema.enum_display_maps[field_name]
                            display_value = enum_display_map.get(raw_value, str(raw_value))
    ...
    def _binary_search_visible_insert_position(self, timestamp: Union[datetime, float]) -> int:
